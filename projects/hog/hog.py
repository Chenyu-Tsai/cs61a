...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    res = [dice() for n in range(num_rolls)]
    return sum(res) if 1 not in res else 1
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    n = max(player_score, opponent_score)
    while n >= 10:
        mod_p = True if player_score % n == 0 else False
        mod_o = True if opponent_score % n == 0 else False
        if mod_p == True and mod_o == True:
            return True
        n -= 1
    return False
