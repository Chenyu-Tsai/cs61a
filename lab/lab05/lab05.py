...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        sprout_leaves_braches = [sprout_leaves(branch, leaves) for branch in branches(t)]
        result = tree(label(t), sprout_leaves_braches)
        return result
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[0+k//2] if k % 2 == 0 else deck[len(deck)//2 + k//2] for k in range(len(deck))]
