...
def balanced(m):
    ...
    if is_planet(m):
        ...
        l_weight, r_weight = total_weight(end(left(m))), total_weight(end(right(m)))
        l_length, r_length = length(left(m)), length(right(m))
        l_torque, r_torque = l_weight * l_length, r_weight * r_length
        return l_torque == r_torque and balanced(end(left(m))) and balanced(end(right(m)))
...
def check_par():
    ...
    True
    """
    r1 = interval(5, 2) # Replace this line!
    r2 = interval(6, 9) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    values = [quad_fn(lower_bound(x)), quad_fn(upper_bound(x))]
    
    extreme_point = -b/(2*a)
    if extreme_point >= lower_bound(x) and extreme_point <= upper_bound(x):
        values.append(quad_fn(extreme_point))
